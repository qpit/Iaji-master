...
class CovarianceMatrix:
    """
    This class describes a covariance matrix as a physical parameter.
    """
    # ----------------------------------------------------------
    def __init__(self, name="Sigma", value=None, real=False, nonnegative=False):
        self.name = name
        self.type = "vector"
        self.symbolic = CovarianceMatrixSymbolic(name=name)
        self.numeric = CovarianceMatrixNumeric(name=name, value=value)
...
class CovarianceMatrixSymbolic(MatrixSymbolic):
    ...
    def expression(self, expression):
        ...
        if expression is not None:
            ...
            try:
                if not self.isCovarianceMatrix()[0]:
                    error_message = "Value of matrix "+self.name.__str__()+" cannot represent a covariance matrix, because it is not square.\n"+self.__str__()
                    self._expression = None
                    self._shape = None
                    raise TypeError(error_message)
                self.expression_changed.emit()  # emit expression changed signal
            except TestFailedError:
                warning_message = "Could not verify that matrix "+self.name.__str__()+" is a covariance matrix.\n" + self.__str__()
...
class CovarianceMatrixNumeric(MatrixNumeric):
    ...
    """
    # ----------------------------------------------------------
    def __init__(self, name="Sigma", value=None):
    ...
    def value(self, value):
        if value is not None:
            ...
            if not self.isCovarianceMatrix()[0]:
                error_message = "Value of matrix "+self.name.__str__()+" does not represent a covariance matrix. \n"+self.__str__()
                self._value = None
                self._shape = None
                self._eigenvalues, self._rank, self._trace, self._determinant = [None for j in range(4)]
                raise TypeError(error_message)
