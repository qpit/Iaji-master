...
c_q_SA, c_p_SA = sympy.symbols("c_{q_{SA}}, c_{p_{SA}}", real=True)
r_1_EB, nu_2_EB = sympy.symbols("r^{(EB)}_1, \\nu^{(EB)}_2", real=True, positive=True)
...
eq3 = sympy.simplify(sympy.Eq(V_PM[0, 0] - c_q_SA**2, V_EB[0, 0]))
eq4 = sympy.simplify(sympy.Eq(V_PM[1, 1] - c_p_SA**2, V_EB[1, 1]))
#Solve the system of equations
system = [eq1, eq2, eq3, eq4]

solution = sympy.solve(system, (nu_2_EB, r_1_EB, R_m, V_p_A), dict=True)
